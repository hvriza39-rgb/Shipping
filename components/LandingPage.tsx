...
export default function LandingPage() {
  ...
  useEffect(() => {
    const obs = new IntersectionObserver((entries) => {
      entries.forEach(e => { if (e.isIntersecting) setVisible(p => new Set([...p, e.target.dataset.s])); });
    }, { threshold: 0.12 });
    document.querySelectorAll("[data-s]").forEach(el => obs.observe(el));
    return () => obs.disconnect();
  }, []);

  const vis = (key, delay = 0) => ({
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .h-title { animation: fadeUp 0.7s ease both; }
        .h-sub   { animation: fadeUp 0.7s 0.12s ease both; }
        .h-cta   { animation: fadeUp 0.7s 0.24s ease both; }
        .h-card  { animation: fadeUp 0.7s 0.18s ease both, float 4.5s 1s ease-in-out infinite; }

        .feat-card { transition: transform 0.2s, box-shadow 0.2s; }
        .feat-card:hover { transform: translateY(-5px) !important; box-shadow: 0 16px 40px rgba(37,99,235,0.12) !important; }
        .btn-p:hover { background: #1D4ED8 !important; }
        .btn-o:hover { background: rgba(255,255,255,0.08) !important; }
        .btn-w:hover { background: #EFF6FF !important; }
        .nav-link:hover { color: rgba(255,255,255,0.9) !important; }
        .foot-link:hover { color: rgba(255,255,255,0.6) !important; }
      `}</style>

      <div style={{ fontFamily: "'Inter', system-ui, sans-serif", color: "#101828" }}>
        ...
        <nav style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <span style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: 16, color: "#fff", letterSpacing: "-0.02em" }}>SwiftShip</span>
          </div>
        ...
        {/* ─── HERO ────────────────────────────────── */}
        <section style={{ minHeight: "100vh", background: "#0C1421", display: "flex", flexDirection: "column", justifyContent: "center", padding: "100px 40px 0", position: "relative", overflow: "hidden" }}>
          {/* Grid bg */}
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", maxWidth: 1160, margin: "0 auto", width: "100%", position: "relative", zIndex: 1, gap: 48 }}>
            ...
            <div style={{ maxWidth: 560 }}>
              <div className="h-title">
                ...
                <h1 style={{ fontFamily: "'Syne', sans-serif", fontSize: 56, fontWeight: 800, color: "#fff", letterSpacing: "-0.04em", lineHeight: 1.07, margin: 0 }}>
                  Every package,<br />
            ...
            <div className="h-card" style={{ flexShrink: 0 }}>
              <div style={{ background: "#131D2D", border: "1px solid rgba(255,255,255,0.09)", borderRadius: 20, padding: 26, width: 330, boxShadow: "0 32px 80px rgba(0,0,0,0.55), 0 0 0 1px rgba(255,255,255,0.04)" }}>
                ...
                {[
                  { label: "Order Placed",     done: true,  time: "Jun 19, 8:30 AM" },
                  { label: "Picked Up",        done: true,  time: "Jun 19, 2:00 PM" },
                  { label: "In Transit",       done: true,  time: "Jun 20, 9:00 AM" },
                  { label: "Out for Delivery", done: true,  time: "Jun 21, 7:30 AM" },
                  { label: "Delivered",        done: true,  time: "Jun 21, 2:22 PM" },
                ].map((step, i, arr) => (
                  <div key={i} style={{ display: "flex", gap: 10 }}>
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", width: 14, flexShrink: 0 }}>
                      <div style={{ width: 7, height: 7, borderRadius: "50%", background: step.done ? "#22C55E" : "rgba(255,255,255,0.12)", marginTop: 3, flexShrink: 0 }} />
                      {i < arr.length - 1 && <div style={{ width: 1.5, flex: 1, minHeight: 12, background: "rgba(255,255,255,0.07)" }} />}
                    </div>
                    <div style={{ paddingBottom: 9 }}>
                      <div style={{ fontSize: 11, fontWeight: 600, color: step.done ? "rgba(255,255,255,0.85)" : "rgba(255,255,255,0.2)" }}>{step.label}</div>
        ...
        <section style={{ background: "#F8F9FB", padding: "96px 40px" }}>
          <div style={{ maxWidth: 1100, margin: "0 auto" }}>
            <div data-s="feats" style={{ textAlign: "center", marginBottom: 52, ...vis("feats") }}>
              ...
              <h2 style={{ fontFamily: "'Syne', sans-serif", fontSize: 36, fontWeight: 800, letterSpacing: "-0.04em", color: "#0C1421", margin: "0 0 12px" }}>Built for people who actually ship things</h2>
        ...
        <section style={{ background: "#fff", padding: "96px 40px" }}>
          <div style={{ maxWidth: 1100, margin: "0 auto" }}>
            <div data-s="how" style={{ marginBottom: 52, ...vis("how") }}>
              ...
              <h2 style={{ fontFamily: "'Syne', sans-serif", fontSize: 36, fontWeight: 800, letterSpacing: "-0.04em", color: "#0C1421" }}>From booking to doorstep</h2>
            </div>
        ...
        <section style={{ position: "relative", height: 460, overflow: "hidden" }}>
          ...
          <div data-s="img" style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", padding: "0 80px", ...vis("img") }}>
            <div style={{ maxWidth: 480 }}>
              <h2 style={{ fontFamily: "'Syne', sans-serif", fontSize: 36, fontWeight: 800, color: "#fff", letterSpacing: "-0.04em", margin: "0 0 14px", lineHeight: 1.15 }}>
            ...
            <div data-s="photos" style={{ display: "flex", gap: 14, ...vis("photos") }}>
              ...
              <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 14 }}>
                <div style={{ flex: 1, borderRadius: 16, overflow: "hidden" }}>
                  <img src="https://images.unsplash.com/photo-1524508762098-b9f8c975d5ec?w=600&q=80" alt="Packages ready to ship" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
                </div>
                <div style={{ flex: 1, borderRadius: 16, overflow: "hidden" }}>
                  <img src="https://images.unsplash.com/photo-1609743522653-52354461eb27?w=600&q=80" alt="Courier at door" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
                </div>
              </div>
            </div>
          </div>
        ...
        <section style={{ background: "#2563EB", padding: "96px 40px" }}>
          <div data-s="cta" style={{ maxWidth: 580, margin: "0 auto", textAlign: "center", ...vis("cta") }}>
            <h2 style={{ fontFamily: "'Syne', sans-serif", fontSize: 40, fontWeight: 800, color: "#fff", letterSpacing: "-0.04em", margin: "0 0 16px", lineHeight: 1.1 }}>
        ...
        <footer style={{ background: "#0C1421", padding: "44px 40px", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
          <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 20 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <span style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: 14, color: "#fff", letterSpacing: "-0.02em" }}>SwiftShip</span>
            </div>
