...
export default function AdminDashboard() {
  ...
  // Location/status update modal state
  ...
  const pushTrackingEvent = async (id: string) => {
    ...
    try {
      ...
      const nextStatus = locForm.status as ShipmentStatus;
      setShipments((prev) =>
        prev.map((s) => {
          if (s.id !== id) return s;
          return {
  ...
  const modalShip = locationModal ? shipments.find(s => s.id === locationModal) : null;
  ...
  return (
    <div style={{ fontFamily: "'Inter', system-ui, sans-serif", minHeight: "100vh", background: "#F8F9FB", color: "#101828" }}>
                ...
                {filtered.map((s, i) => {
                  ...
                  return (
                    <tr key={s.id} style={{ borderBottom: i < filtered.length - 1 ? "1px solid #F2F4F7" : "none", background: isDrawerOpen ? "#FAFBFF" : "transparent" }}>
                      ...
                      {/* Actions: Update + View (single cell, no nesting) */}
      ...
      {/* LOCATION / STATUS UPDATE MODAL — rendered independently of the drawer
          so it still shows even though the drawer is closed when this opens */}
      {modalShip && (() => {
        const transitions = STATUS_TRANSITIONS[modalShip.status] ?? [];
        return (
            ...
            <div style={{ position: "relative", background: "#fff", borderRadius: 14, padding: 28, width: 440, boxShadow: "0 24px 64px rgba(0,0,0,0.2)", zIndex: 1 }}>
              ...
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 20 }}>
                <div>
                  ...
                  <div style={{ fontFamily: "monospace", fontSize: 11, color: "#2563EB", fontWeight: 700, marginTop: 4 }}>{modalShip.trackingNumber}</div>
                </div>
                <button
                  onClick={() => { setLocationModal(null); setLocError(""); }}
              ...
              <div style={{ background: "#F8F9FB", borderRadius: 8, padding: "10px 14px", marginBottom: 18, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                ...
                <StatusBadge status={modalShip.status} />
              </div>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                >
                  ...
                  <option value={modalShip.status}>{STATUS_META[modalShip.status]?.label ?? modalShip.status} (location update only)</option>
                </select>
              </div>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                ...
                <button
                  onClick={() => pushTrackingEvent(modalShip.id)}
